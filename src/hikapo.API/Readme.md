...
WORKDIR /src
COPY ["hikapo.API.csproj", "hikapo.API/"]
RUN dotnet restore "hikapo.API/hikapo.API.csproj"

WORKDIR "/src/hikapo.API"
COPY . .
RUN dotnet build "hikapo.API.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "hikapo.API.csproj" \
...
Execute unit tests
```
cd "hikapo.API/test/hikapo.API.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "hikapo.API/src/hikapo.API"
